...
if ( $donations ) : ?>
	...
	$table_headings = array(
		'id'             => esc_html( '[:en]ID[:ru]Номер[:es]ID[:]'),
		'date'           => esc_html( '[:en]Date[:ru]Дата[:es]Fecha[:]'),
		'donor'          => esc_html( '[:en]Donor[:ru]Донор[:es]Donante[:]'),
		'amount'         => esc_html( '[:en]Amount[:ru]Сумма[:es]Cantidad[:]'),
		'status'         => esc_html( '[:en]Status[:ru]Статус[:es]Estado[:]'),
		'payment_method' => esc_html( '[:en]Payment Method[:ru]Способ оплаты[:es]Método de pago[:]'),
		'details'        => esc_html( '[:en]Details[:ru]Детали[:es]Detalles[:]'),
	);
	?>
					...
					<td class="give-donation-details">
						...
						if ( 'publish' !== $post->post_status && 'subscription' !== $post->post_status ) :
							...
							echo sprintf(
								...
								esc_html( '[:en]View Receipt &raquo;[:ru]Посмотреть квитанцию &raquo;[:es]Ver recibo &raquo;[:]'),
								esc_html( $table_headings['details'] )
							);
