...
$give_receipt_args['donation_receipt']['donor'] = array(
	'name'    => __( '[:en]Donor[:ru]Донор[:es]Donante[:]', 'give' ),
	'value'   => $full_name,
...
$give_receipt_args['donation_receipt']['company_name'] = array(
	'name'    => __( '[:en]Company Name[:ru]Название компании[:es]Nombre de empresa[:]', 'give' ),
...
$give_receipt_args['donation_receipt']['date'] = array(
	'name'    => __( '[:en]Date[:ru]Дата[:es]Fecha[:]', 'give' ),
...
$give_receipt_args['donation_receipt']['total_donation'] = array(
	'name'    => __( '[:en]Total Donation[:ru]Сумма пожертвования[:es]Donación total[:]', 'give' ),
...
if ( filter_var( $give_receipt_args['status_notice'], FILTER_VALIDATE_BOOLEAN ) ) {
	...
	switch ( $status ) {
		case 'publish':
			$notice_message = __( '[:en]Payment Complete: Thank you for your donation.[:ru]Оплата завершена. Cпасибо за ваше пожертвование.[:es]Payment Complete: Thank you for your donation.[:]', 'give' );
			$notice_type    = 'success';
			break;
		case 'pending':
...
<table id="give_donation_receipt" class="give-table">
	...
	<tr>
		<th scope="colgroup" colspan="2">
			<span class="give-receipt-thead-text"><?php esc_html_e( '[:en]Donation Receipt[:ru]Информация о пожертвовании[:es]Donation Receipt[:]', 'give' ) ?></span>
		</th>
	</tr>
	<?php
	/**
