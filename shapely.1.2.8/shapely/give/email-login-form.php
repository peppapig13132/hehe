...
if ( true === $is_form_required ) {
	...
	<div class="give-form">
		<form method="post" id="give-email-access-form">
			...
			<label for="give-email"><?php esc_attr_e( 'Email:', 'give' ); ?></label>
			<input id="give-email" type="email" name="give_email" value=""
					placeholder="<?php esc_attr_e( 'Email', 'give' ); ?>"/>
			...
			<input type="submit" class="give-submit" value="<?php esc_attr_e( '[:en]Verify Email[:ru]Подтвердить Email[:es]Verificar Email[:]', 'give' ); ?>"/>
		</form>
	</div>
	<?php
}
